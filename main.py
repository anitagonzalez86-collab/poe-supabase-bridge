...
def handle_chat():
    ...
    if not dni_usuario or not pregunta_usuario:
        return {"error": "Falta el DNI o la pregunta."}, 400
    ...
    try:
        ...
        if not ficha_paciente:
            return {"error": "FICHA NO ENCONTRADA. Verifica el DNI."}, 404
    except Exception as e:
        return {"error": "Error al buscar la ficha en Supabase."}, 500

    # 2. BUSCAR CONTEXTO RELEVANTE PARA LA PREGUNTA ESPECÍFICA
    try:
        # Usamos la pregunta del usuario para la búsqueda semántica
    ...
    # 3. PREPARAR Y ENVIAR LA PREGUNTA A GOOGLE AI
    try:
        prompt = f"""
        ...
        **Respuesta:**
        ...
        return {"error": "Error al generar la respuesta con la IA de Google."}, 500